...
class quiz_teacheroverview_report extends quiz_attempts_report {
    ...
    public function display($quiz, $cm, $course) {
        ...
        if ($DB->record_exists('quiz_grades', array('quiz' => $quiz->id))) {
            ...
            $data = quiz_teacheroverview_grade_bands($bandwidth, $bands, $quiz->id,
                $currentgroup, new \core\dml\sql_join());
        ...
        if (!$table->is_downloading()) {
            ...
            $competencyreport->lastaccess = $this->lastaccess;
            $skills = $competencyreport->get_brief_competencies($quiz, $cm, $course);
            if (count($skills) != 0) {
                // Competencyoverview report button.
                $competencybuttonurl  = new moodle_url($CFG->wwwroot . '/mod/quiz/report.php?id=' . $cm->id . '&mode=competencyoverview', array('display' => 'full', 'lastaccess' => $this->lastaccess));
                $competencybuttonname = get_string('competencyoverview', 'quiz_teacheroverview');
                ...
                echo $OUTPUT->render_from_template('quiz_teacheroverview/competencies', $compcontext);
            }
        }
        ...
        if ($hasquestions && ($hasstudents || $options->attempts == self::ALL_WITH)) {
            ...
            if (has_capability('mod/quiz:deleteattempts', $this->context)) {
                $buttons .= '<input type="submit" form="attemptsform" class="btn btn-primary m-r-1"
                id="deleteattemptsbutton" name="delete" value="' .
                get_string('deleteselected', 'quiz_teacheroverview') . '"/>';

                // TODO: Not sure why we need this? (nadavkav) PTL
                /*
            ...
            if (has_capability('mod/quiz:regrade', $this->context)) {
                $buttons .= '<input type="submit" form="attemptsform" class="btn btn-primary m-r-1" name="regrade" value="' .
        ...
        if (!$table->is_downloading()) {
            ...
            if (has_capability('mod/quiz:regrade', $this->context)) {
                ...
                echo '<input type="submit" class="btn btn-primary m-r-1" name="regradeall" value="' . $regradealllabel . '"/>';

                if ($this->displayfull) {
                    echo '<input type="submit" class="btn btn-primary m-r-1" name="regradealldry" value="' .
                        $regradealldrylabel . '"/>';
                }
                if ($regradesneeded) {
                    echo '<input type="submit" class="btn btn-primary m-r-1" name="regradealldrydo" value="' .
                        $regradealldrydolabel . '"/>';
                }
                echo '</div>';
                echo '</form>';
                ...
                echo '<input type="button" id="sendmessage" form="participantsform"
                    class="btn btn-primary m-r-1" name="sendmessage" value="' . $sendmessagelabel . '"/>';

                echo '</div>';
            }
        }

        $PAGE->requires->js_call_amd('quiz_teacheroverview/participants', 'load', [$course->id]);

        $PAGE->requires->js_call_amd('quiz_teacheroverview/charts', 'init', array($block3j, $block4j, $quiz->grade, $cm->id));
    ...
    protected function get_questions_stat($quiz, $currentgroup, $usersfinished) {
        ...
        foreach ($slots as $slot => $slotrecord) {
            ...
            $qsql = "
                ...
                LEFT JOIN {groups_members} AS gm ON (qas.userid = gm.userid)
                ...
                AND (quiza.userid, quiza.attempt) IN
                    (
                        ...
                        FROM {quiz_attempts} WHERE state = 'finished' AND quiz = :quiz GROUP BY userid
                    )
                AND qas.fraction = 1
            ";
